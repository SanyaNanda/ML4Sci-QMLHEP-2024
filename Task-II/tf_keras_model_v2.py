...
def get_particle_net_lite_v2(num_classes, input_shapes):
    points = keras.Input(name='points', shape=input_shapes['points'])
    features = keras.Input(name='features', shape=input_shapes['features']) if 'features' in input_shapes else None
    mask = keras.Input(name='mask', shape=input_shapes['mask']) if 'mask' in input_shapes else None

    # Branch 1: Processing spatial information
    spatial_branch = layers.Conv1D(64, 1, activation='relu')(points)
    spatial_branch = layers.BatchNormalization()(spatial_branch)
    spatial_branch = layers.MaxPooling1D(pool_size=2)(spatial_branch)
    spatial_branch = layers.Conv1D(128, 1, activation='relu')(spatial_branch)
    spatial_branch = layers.BatchNormalization()(spatial_branch)
    spatial_branch = layers.MaxPooling1D(pool_size=2)(spatial_branch)
    spatial_branch = layers.Conv1D(256, 1, activation='relu')(spatial_branch)
    spatial_branch = layers.BatchNormalization()(spatial_branch)
    spatial_branch = layers.GlobalAveragePooling1D()(spatial_branch)

    # Branch 2: Processing channel-wise information
    channel_branch = layers.Conv1D(64, 1, activation='relu')(features)
    channel_branch = layers.BatchNormalization()(channel_branch)
    channel_branch = layers.Conv1D(128, 1, activation='relu')(channel_branch)
    channel_branch = layers.BatchNormalization()(channel_branch)
    channel_branch = layers.Conv1D(256, 1, activation='relu')(channel_branch)
    channel_branch = layers.BatchNormalization()(channel_branch)
    channel_branch = layers.GlobalMaxPooling1D()(channel_branch)

    # Combine branches
    combined = layers.Concatenate()([spatial_branch, channel_branch])
    ...
    return keras.Model(inputs=[points, features, mask], outputs=outputs, name='ParticleNetLiteV2')
